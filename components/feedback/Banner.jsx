...
export function Banner({ tone = "info", title, children, action, onDismiss, style = {} }) {
  ...
  return (
    <div role="status" style={{ display: "flex", alignItems: "flex-start", gap: 12, background: bg, borderRadius: "var(--radius-lg)", padding: "14px 16px", borderLeft: `3px solid ${fg}`, ...style }}>
      <span style={{ marginTop: 1, color: fg, flex: "none" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {title && <div style={{ fontFamily: "var(--font-sans)", fontSize: "var(--text-sm)", fontWeight: "var(--fw-bold)", color: "var(--ink-900)" }}>{title}</div>}
        {children && <div style={{ fontFamily: "var(--font-sans)", fontSize: "var(--text-sm)", color: "var(--ink-700)", marginTop: title ? 2 : 0, lineHeight: "var(--lh-normal)" }}>{children}</div>}
        {action && <div style={{ marginTop: 10 }}>{action}</div>}
      </div>
      {onDismiss && (
        <button onClick={onDismiss} aria-label="Dismiss" style={{ border: "none", background: "transparent", cursor: "pointer", color: fg, opacity: 0.7, padding: 0, display: "inline-flex" }}>
