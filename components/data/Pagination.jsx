...
export function Pagination({ page = 1, pageCount = 1, onChange, style = {} }) {
  ...
  const chev = (d) => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d={d} /></svg>;
  const cell = (content, { key, active, disabled, onClick } = {}) => (
    <button key={key} disabled={disabled} onClick={onClick} aria-current={active ? "page" : undefined}
      style={{
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, ...style }}>
      {cell(chev("M15 6l-6 6 6 6"), { key: "prev", disabled: page <= 1, onClick: () => go(page - 1) })}
      {nums.map((n, i) => n === "…"
        ? <span key={"e" + i} style={{ padding: "0 4px", color: "var(--text-muted)" }}>…</span>
        : cell(n, { key: n, active: n === page, onClick: () => go(n) }))}
      {cell(chev("M9 6l6 6-6 6"), { key: "next", disabled: page >= pageCount, onClick: () => go(page + 1) })}
    </div>
  );
}
